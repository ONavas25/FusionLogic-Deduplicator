...
def log_memory_usage(stage_name):
    ...
    Displays memory usage, available RAM, and garbage collection activity.
    """
    try:
...
def batch_processing_mode(working_directory, input_file_name, output_directory):
    """
    Perform batch processing that includes cleaning, duplicate extraction,
    spot checks, and file renaming/moving in sequence.
    """
    try:
...
def initialize_logging(working_directory):
    """
    Initialize the logging system to capture program output in a log file.
    """
    try:
        log_file_path = os.path.join(working_directory, "logfile.txt")
        sys.stdout = Logger(log_file_path)
...
class Logger:
    ...
    def __init__(self, log_file_path):
        ...
        self.console = sys.__stdout__ or sys.stderr  
...
def main():
    ...
    while True:
        ...
        if option == '1':
            ...
            # ✅ Log memory usage after deduplication
            log_memory_usage("Post Deduplication")
            ...
            if progress_data["processed_files"]["1"]:
                if option == '2':
                    ...
                    # ✅ Log memory usage after duplicate extraction
                    log_memory_usage("Post Duplicate Extraction")

                elif option == '3':
                    perform_spot_checks(progress_data, progress_file)
                    
                    # ✅ Log memory usage after spot checks
                    log_memory_usage("Post Spot Checks")

                elif option == '4':
                    search_string_in_file()
                    
                    # ✅ Log memory usage after search execution
                    log_memory_usage("Post Search Execution")

                elif option == '5':
                    rename_and_move_file(progress_data, progress_file)
                    
                    # ✅ Log memory usage after renaming/moving files
                    log_memory_usage("Post Rename & Move")
                ...
                print("DEBUG: Error: Option 1 must be completed first.")
                sys.stdout.flush()
        elif option == '7':
            
            # ✅ Log memory usage before exiting the program
